...
const BookingForm: React.FC<BookingFormProps> = ({ activeTab, onTabChange }) => {
  ...
  const [agenciesData, setAgenciesData] = useState<Record<string, { ticket: string[]; parcel: string[]; code: string }>>({});
  const [callCenterNumbers, setCallCenterNumbers] = useState<{ num: string; code: string }[]>([]);

  useEffect(() => {
    api
      .get<AgencyApi[]>('/agencies')
      .then((agencies) => {
        const map: Record<string, { ticket: string[]; parcel: string[]; code: string }> = {};
        agencies.forEach((a) => {
          map[a.city] = { ticket: a.ticketPhones, parcel: a.parcelPhones, code: a.countryCode };
        });
        setAgenciesData(map);
      })
      .catch(() => setAgenciesData({}));

    api
      .get<SupportNumberApi[]>('/support-numbers')
      .then((numbers) => {
        setCallCenterNumbers(numbers.filter((n) => n.category === 'TICKET').map((n) => ({ num: n.phone, code: '+228' })));
      })
      .catch(() => setCallCenterNumbers([]));
  }, []);
      ...
      <div className="flex bg-gray-50 border-b border-gray-200 p-1.5">
        {tabs.map((tab) => (
          <div key={tab.id} className="relative flex-1">
            ...
            {activeTab === tab.id && (
              <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-0 h-0 border-l-[6px] border-l-transparent border-r-[6px] border-r-transparent border-t-[6px] border-t-[var(--brand-dark)] z-10"></div>
            )}
          </div>
        ))}
      </div>
      ...
      <div className="p-8 space-y-6 min-h-[420px]">
            ...
            {ticketAction === 'choice' && (
              <div className="space-y-4 py-4">
                <button 
                  ...
                  className="w-full flex items-center justify-between p-6 bg-gray-50 hover:bg-[rgb(var(--brand-dark-rgb)/5%)] border border-gray-100 rounded-2xl transition-all group"
                >
                  ...
                  <ChevronDown size={20} className="text-gray-300 -rotate-90 group-hover:text-[var(--brand-dark)] transition-all" />
                </button>
            ...
            {ticketAction === 'reservation' && (
              <div className="animate-in fade-in slide-in-from-right-4 duration-300 space-y-6">
                <button 
                  ...
                  className="flex items-center gap-2 text-[var(--brand-dark)] text-[10px] font-black uppercase tracking-widest hover:opacity-70 transition-opacity mb-2"
                >
                ...
                <div className="grid grid-cols-1 gap-2 max-h-[250px] overflow-y-auto no-scrollbar pr-1">
                  {callCenterNumbers.map((item, i) => (
                    <a 
                      ...
                      className="flex items-center justify-between p-4 bg-gray-50 border border-gray-100 rounded-xl hover:bg-[var(--brand-dark)] group transition-all"
                    >
                      <div className="flex items-center gap-3">
                        <Phone size={16} className="text-[var(--brand-dark)] group-hover:text-white" />
                        <span className="text-[13px] font-black text-gray-900 group-hover:text-white tracking-widest">{item.code} {item.num}</span>
                      </div>
                      <span className="text-[9px] font-black text-[var(--brand-dark)] group-hover:text-white uppercase tracking-widest bg-white group-hover:bg-white/20 px-2 py-1 rounded shadow-sm">Appeler</span>
                    </a>
                  ))}
                </div>
                <ImportantNote />
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      <select 
                        className="w-full px-4 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm outline-none appearance-none pr-10 cursor-pointer font-bold focus:border-[var(--brand-dark)]"
                  ...
                  {selectedAgency && (
                    <div className="animate-in zoom-in-95 duration-500 bg-[rgb(var(--brand-dark-rgb)/5%)] p-6 rounded-2xl border border-[rgb(var(--brand-dark-rgb)/10%)] space-y-4">
                      <p className="text-xs text-gray-500 font-medium text-center">Pour annuler ou reporter au départ de <span className="font-black text-[var(--brand-dark)] uppercase">{selectedAgency}</span>, contactez le guichet :</p>
                      <div className="grid gap-2">
                        {agenciesData[selectedAgency].ticket.map((num, i) => (
                          <a 
                            key={i}
                            href={`tel:${agenciesData[selectedAgency].code}${num.replace(/\s/g, '')}`}
                            className="flex items-center justify-between px-6 py-4 bg-white border border-[rgb(var(--brand-dark-rgb)/20%)] rounded-2xl shadow-sm hover:bg-[var(--brand-dark)] group transition-all"
                          >
                            <div className="flex items-center gap-3">
                              <Phone size={18} className="text-[var(--brand-dark)] group-hover:text-white" />
                              <span className="text-base font-black text-[var(--brand-dark)] group-hover:text-white tracking-widest">{agenciesData[selectedAgency].code} {num}</span>
                            </div>
                            <span className="text-[9px] font-black uppercase text-[var(--brand-dark)] group-hover:text-white">Appeler</span>
                          </a>
                        ))}
                      </div>
                    </div>
        ...
        {activeTab === ServiceTab.COLIS && (
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-400 space-y-8">
            <div className="space-y-2">
              ...
              <div className="relative group">
                <select 
                  className="w-full px-4 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm outline-none appearance-none pr-10 cursor-pointer font-bold focus:border-[var(--brand-dark)] transition-all"
            ...
            {selectedParcelAgency ? (
              <div className="animate-in zoom-in-95 duration-500 space-y-6">
                <div className="bg-[rgb(var(--brand-dark-rgb)/5%)] p-10 rounded-[2.5rem] border border-[rgb(var(--brand-dark-rgb)/10%)] text-center space-y-8 shadow-inner">
                  <div className="w-20 h-20 bg-white rounded-3xl flex items-center justify-center text-[var(--brand-dark)] shadow-md mx-auto transform -rotate-3">
                    <Box size={40} />
                  </div>
                  
                  <div className="space-y-3">
                    <h4 className="text-[10px] font-black text-[var(--brand-dark)] uppercase tracking-[0.3em]">Agence Nagode {selectedParcelAgency}</h4>
                  ...
                  <div className="grid gap-3">
                    {agenciesData[selectedParcelAgency].parcel.map((num, i) => (
                      <a 
                        ...
                        className="flex items-center justify-center gap-4 py-5 bg-[var(--brand-dark)] text-white rounded-[1.5rem] shadow-lg hover:bg-[var(--brand-dark-hover)] hover:scale-105 transition-all group"
                      >
        ...
        {activeTab === ServiceTab.ARGENT && (
           <div className="flex flex-col items-center justify-center py-16 text-center animate-in zoom-in-95 duration-600">
             ...
             <h3 className="text-2xl font-black text-[var(--brand-dark)] mb-2 uppercase tracking-tight">Nagode Pay</h3>
